...
for file_name in os.listdir(folder_path):
    ...
    # plt.title(f'Zipf-Law for {file_name}', fontsize=18)  # 标题
    # plt.xlabel('rank', fontsize=18)  # 排名
    # plt.ylabel('freq', fontsize=18)  # 频度
    # plt.yscale('log')  # 设置纵坐标的缩放
    # plt.xscale('log')  # 设置横坐标的缩放
    # plt.plot(x, freq_list, 'r')  # 绘图
    #
    # fit = np.polyfit(np.log(x), np.log(freq_list), 1)
    # fit_fn = np.poly1d(fit)
    # plt.plot(x, np.exp(fit_fn(np.log(x))), '--k')
    #
    # plt.savefig(f'./{file_name.split(".")[0]}_Zipf_Law.jpg')  # 保存图片
    # plt.show()
#画在一张图上
    # 绘制曲线
    freq_list = sorted(counts.values(), reverse=True)  # 词频列表
